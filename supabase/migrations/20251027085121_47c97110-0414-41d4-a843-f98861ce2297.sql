...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'store_region_links_store_id_region_id_key'
  ) THEN
    ALTER TABLE public.store_region_links ADD CONSTRAINT store_region_links_store_id_region_id_key UNIQUE(store_id, region_id);
  END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_store_region_links_store ON public.store_region_links(store_id);
CREATE INDEX IF NOT EXISTS idx_store_region_links_region ON public.store_region_links(region_id);
CREATE INDEX IF NOT EXISTS idx_items_group ON public.items(group_id);
CREATE INDEX IF NOT EXISTS idx_item_groups_parent ON public.item_groups(parent_group_id);
